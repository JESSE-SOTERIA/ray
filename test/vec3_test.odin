...
test_negate :: proc(t: ^testing.T) {
	...
	testing.expect(t, v == Vec3{-3.0, -4.0, -5.0}, "negate function failed to negate")
}
...
test_get_index :: proc(t: ^testing.T) {
	...
	testing.expect(t, get_index(v, 3) == 0.0, "get_index worked with a value greater than 2")
}
...
@(test)
test_get_veclength :: proc(t: ^testing) {
	v := init_with_values(6.0, 4.0, 8.0)
	len := get_veclen(v)
	testing.expect(
		t,
		math.abs(len - 10.7) < 1e-6,
		"get_vec_len does not accurately evaluate the magnitude of vectors.",
	)
}

@(test)
test_vec_len_squared :: proc(t: ^testing) {
	v := init_with_values(6.0, 4.0, 8.0)
	len := get_veclen(v)
